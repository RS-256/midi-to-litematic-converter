...
export function writeLitematic( input: LitematicInput ): Uint8Array {
  ...
  const totalBlocks = normalizedRegions.reduce( ( sum, region ) => sum + region.totalBlocks, 0 )
  const totalVolume = normalizedRegions.reduce( ( sum, region ) => sum + region.volume, 0 )
  ...
  const root: Record< string, NbtTag > = {
    Regions: {
      type: "compound",
      value: Object.fromEntries( normalizedRegions.map( ( region ) => [ region.name, regionToNbt( region ) ] ) )
    },
    SubVersion: {
      type: "int",
      value: 1
...
function getEnclosingSize( regions: NormalizedRegion[] ): Vec3 {
  ...
  const maxX = Math.max( ...regions.map( ( region ) => region.position.x + region.size.x - 1 ) )
  const maxY = Math.max( ...regions.map( ( region ) => region.position.y + region.size.y - 1 ) )
  const maxZ = Math.max( ...regions.map( ( region ) => region.position.z + region.size.z - 1 ) )
