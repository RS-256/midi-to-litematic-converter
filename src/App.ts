...
export class App {
  ...
  private getElements(): AppElements {
    return {
      ...
      litematicVersionSelect: getElement< HTMLSelectElement >( "#litematic-version-select", this.root ),
      blocksPerQuarterInput: getElement< HTMLInputElement >( "#blocks-per-quarter-input", this.root ),
      startMeasureOffsetInput: getElement< HTMLInputElement >( "#start-measure-offset-input", this.root ),
      repeaterBaseBlockInput: getElement< HTMLInputElement >( "#repeater-base-block-input", this.root ),
      pianoRollZoomInput: getElement< HTMLInputElement >( "#piano-roll-zoom-input", this.root ),
      pianoRollZoomValue: getElement< HTMLSpanElement >( "#piano-roll-zoom-value", this.root ),
  ...
  private bindGlobalEvents(): void {
    ...
    this.elements.litematicVersionSelect.addEventListener( "change", () => {
      ...
      if ( version !== 6 && version !== 7 ) {
        this.elements.litematicVersionSelect.value = String( this.exportSettings.litematicVersion )
        return
      }
    ...
    this.elements.blocksPerQuarterInput.addEventListener( "change", () => {
      ...
      if ( ! Number.isFinite( value ) || value <= 0 ) {
        this.elements.blocksPerQuarterInput.value = String( this.exportSettings.blocksPerQuarterNote )
        return
      }
    ...
    this.elements.startMeasureOffsetInput.addEventListener( "change", () => {
      ...
      if ( ! Number.isFinite( value ) || value < 0 ) {
        this.elements.startMeasureOffsetInput.value = String( this.exportSettings.startMeasureOffset )
        return
      }
    ...
    this.elements.repeaterBaseBlockInput.addEventListener( "change", () => {
      ...
      if ( ! blockId ) {
        this.elements.repeaterBaseBlockInput.value = this.exportSettings.repeaterBaseBlockId
        return
      }
    ...
    this.elements.pianoRollZoomInput.addEventListener( "input", () => {
      ...
      this.elements.pianoRollZoomValue.textContent = `${ Math.round( this.pianoRollZoom * 100 ) }%`
  ...
  private async loadSelectedMidi(): Promise< void > {
    ...
    try {
      ...
      this.elements.pianoRoll.textContent = error instanceof Error ? error.message : String( error )
  ...
  private updateDownloadButtonDisabled(): void {
    this.elements.downloadLitematicButton.disabled = this.isLoadingMidi || this.isUpdatingExportSettings
  }
  ...
  private renderMidiSummary(): void {
    ...
    const totalNotes = this.loadedTracks.reduce( ( sum, track ) => sum + track.notes.length, 0 )
  ...
  private renderTrackList(): void {
    ...
    this.elements.trackList.querySelectorAll< HTMLButtonElement >( ".track-select-button" ).forEach( ( button ) => {
      button.addEventListener( "click", () => {
        this.selectTrack( Number( button.dataset.trackIndex ) )
      } )
    } )

    this.elements.trackList.querySelectorAll< HTMLInputElement >( ".track-visible-checkbox" ).forEach( ( checkbox ) => {
    ...
    this.elements.trackList.querySelectorAll< HTMLInputElement >( ".track-export-checkbox" ).forEach( ( checkbox ) => {
      checkbox.addEventListener( "change", () => {
        const trackIndex = Number( checkbox.dataset.trackIndex )
        const settings = this.getTrackSettings( trackIndex )
  ...
  private renderPianoRoll(): void {
    const previousScroll = this.elements.pianoRoll.querySelector< HTMLDivElement >( ".piano-scroll" )
    ...
    const rightPad = 80
    const bottomPad = 24
    const minMidi = Math.max( 0, Math.min( ...visibleNotes.map( ( { note } ) => note.midi ) ) - 2 )
    const maxMidi = Math.min( 127, Math.max( ...visibleNotes.map( ( { note } ) => note.midi ) ) + 2 )
    const maxTick = Math.max( ...visibleNotes.map( ( { note } ) => note.ticks + note.durationTicks ) )
    ...
    const width = leftPad + maxQuarter * this.exportSettings.blocksPerQuarterNote * pxPerBlock + rightPad
    ...
    const grid = renderPianoGridHtml( {
      ...
      maxMidi,
      height,
      verticalLines: this.getPianoRollVerticalLines( pianoRollTickOffset, gridEndTicks ),
    ...
    const notesSvg = visibleNotes
      .map( ( { track, note } ) => {
        ...
        const noteWidth = Math.max(
          4,
          ( visibleDurationTicks / this.currentPpq ) * this.exportSettings.blocksPerQuarterNote * pxPerBlock
        )
    ...
    const nextScroll = this.elements.pianoRoll.querySelector< HTMLDivElement >( ".piano-scroll" )
    ...
    this.elements.pianoRoll.querySelector< SVGSVGElement >( "svg" )?.addEventListener( "click", ( event ) => {
  ...
  private updateTrackSelectionStyles( previousTrackIndex: number | null, selectedTrackIndex: number | null ): void {
  ...
  private updatePianoRollSelectionStyles( previousTrackIndex: number | null, selectedTrackIndex: number | null ): void {
  ...
  private renderSelectedTrackSettings(): void {
    ...
    const track = this.loadedTracks.find( ( candidate ) => candidate.trackIndex === this.selectedTrackIndex )

    if ( ! track ) {
      this.elements.selectedTrackSettings.textContent = "Selected track was not found."
      return
    }
    ...
    this.elements.selectedTrackSettings.innerHTML = renderNormalTrackSettingsHtml( track, settings )

    this.bindNormalTrackSettings( settings )
  }

  private bindNormalTrackSettings( settings: TrackSettings ): void {
    getElement< HTMLSelectElement >( "#selected-base-midi", this.elements.selectedTrackSettings ).addEventListener(
      "change",
      ( event ) => {
    ...
    getElement< HTMLInputElement >( "#normal-block-input", this.elements.selectedTrackSettings ).addEventListener(
      "change",
      ( event ) => {
    ...
    getElement< HTMLInputElement >(
      ...
      settings.highOverflowBlockId = ( event.target as HTMLInputElement ).value.trim()
      this.renderAll()
    } )

    getElement< HTMLInputElement >( "#low-overflow-block-input", this.elements.selectedTrackSettings ).addEventListener(
      "change",
      ( event ) => {
        settings.lowOverflowBlockId = ( event.target as HTMLInputElement ).value.trim()
        this.renderAll()
      }
    )
  }

  private renderPercussionTrackSettings( track: TrackData, settings: TrackSettings ): void {
    this.elements.selectedTrackSettings.innerHTML = renderPercussionTrackSettingsHtml( track, settings )

    this.bindPercussionTrackSettings( track, settings )
  }

  private bindPercussionTrackSettings( track: TrackData, settings: TrackSettings ): void {
    getElement< HTMLButtonElement >(
      ...
      settings.percussionMappings = applyPercussionPreset( settings.percussionMappings, presetId )
  ...
  private downloadLitematic(): void {
    ...
    const bytes = writeLitematic( {
      ...
      litematicVersion: this.exportSettings.litematicVersion,
      minecraftDataVersion: MINECRAFT_DATA_VERSION_BY_LITEMATIC_VERSION[ this.exportSettings.litematicVersion ],
  ...
  private getMeterLineTicks( endTick: number, lineType: "measure" | "beat" ): number[] {
    const timeSignatures = this.getTimeSignatures()
    const lines: number[] = []
    const finiteEndTick = Number.isFinite( endTick ) ? endTick : this.getMaxTrackEndTick() + this.currentPpq * 16
    ...
    for ( let index = 0; index < timeSignatures.length; index++ ) {
      ...
      const segmentEndTick = Math.min( nextSignature?.ticks ?? finiteEndTick, finiteEndTick )
      const stepTicks =
        lineType === "measure" ? this.getMeasureLengthTicks( signature ) : this.getBeatLengthTicks( signature )

      if ( stepTicks <= 0 ) {
        continue
      }

      for ( let tick = signature.ticks; tick <= segmentEndTick; tick += stepTicks ) {
  ...
  private getMeasureLengthTicks( timeSignature: MidiTimeSignature ): number {
    return Math.round( this.currentPpq * timeSignature.numerator * ( 4 / timeSignature.denominator ) )
  }
  ...
  private getTimeSignatures(): MidiTimeSignature[] {
    const timeSignatures = ( this.currentMidiMeta.timeSignatures ?? [] )
      .filter( ( timeSignature ) => {
        return timeSignature.numerator > 0 && timeSignature.denominator > 0 && Number.isFinite( timeSignature.ticks )
      } )
  ...
  private getMaxTrackEndTick(): number {
    return Math.max(
      0,
      ...this.loadedTracks.flatMap( ( track ) => track.notes.map( ( note ) => note.ticks + note.durationTicks ) )
    )
  }
