...
export function createDefaultPercussionMappings( track: TrackData ): PercussionMapping[] {
  if ( ! track.isPercussion ) {
    return []
  }

  const midiValues = Array.from( new Set( track.notes.map( ( note ) => note.midi ) ) ).sort( ( a, b ) => a - b )

  const mappings = midiValues.map( ( midi ) => ( {
    midi,
    enabled: true,
  ...
  return applyPercussionPreset( mappings, "genericMinecraft" )
}
